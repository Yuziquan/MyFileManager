...
6. 移动（也就是剪切+粘贴）操作中，对于文件，可在不同驱动器间移动（因为FileInfo.MoveTo()支持不同的驱动器间文件的移动）；对于文件夹，则不能在不同驱动器间移动（因为DirectoryInfo.MoveTo()不支持不同的驱动器间目录的移动）；
7. 快速搜索中，因为对于系统盘没有权限访问，导致无法对系统盘进行搜索；
8. 新建一个文件之后，如果马上双击打开查看，则会提示”另一个程序正在使用此文件，进程无法访问“。如果再刷新几次，又能正常打开查看；
9. 不能实现图片预览;
10. 还未实现主窗体右边文件列表的”排序方式“功能；
11. 还未引入快捷键功能；
